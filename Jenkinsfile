pipeline {
    ...
    post {
         success { 
            sh  ("""
                curl -s -X POST https://api.telegram.org/bot${TOKEN}/sendMessage -d chat_id=${CHAT_ID} -d parse_mode=markdown -d text='*https://github.com/yurakorabel/task9_jenkins name*: task9_jenkins \n *Branch*: [$GIT_BRANCH](https://github.com/yurakorabel/task9_jenkins) \n *Build* : [OK](${BUILD_URL}consoleFull)'
            """)
         }
